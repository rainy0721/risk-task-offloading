...
function results = GOFF(params)
    ...
    noise = 10^(noisePower_dBm/10) / 1000;
     % Initialize b 
    b = initialize(numBS,numVehicles,params.Dj);
    ...
    while ~converged
        ...
        for userIndex = random_user_order     
            b_old = b;
            for locationIndex = 1:(numBS + numVehicles)
                decisions = zeros(1, numBS + numVehicles);
                decisions(locationIndex) = 1;
                b(userIndex,:) = decisions;
                expected_utility_1(userIndex,locationIndex) = utility_function(b(userIndex,:),userIndex,b,avg_rate_userIndex_locationIndex,power,numBS,numVehicles,params.kn,params.fj,params.f_j_R, params.T_j_tol, params.E_j_tol,params.F_j_V, params.cj,params.E_i_p,params.an)
            end
            [max_utility, max_server_index] = max(expected_utility_1(userIndex, :))
            expected_utility(userIndex) = max_utility
            decision = zeros(1, numBS + numVehicles);
            decision(max_server_index) = 1;
            b(userIndex,:) = decision;
             for locationIndex = 1:(numBS + numVehicles)
                if locationIndex <= numBS
                    O_userIndex_locationIndex(userIndex,locationIndex) = (b(userIndex,locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex) + params.cj(userIndex,1) * b(userIndex,locationIndex) / params.f_j_R) / params.T_j_tol(userIndex) + ...
                   (power * b(userIndex,locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex)) / params.E_j_tol(userIndex);
        ...
        if flag == Ns 
           ...
           save('GOFF_expected_utility_converging.mat', 'expected_utility_converging');
           disp(['当前迭代次数: ' num2str(iterations)]);  
        end
    end
