% simulation.m
function results = nonProsTheor(params)
    numVehicles = 4;%车辆数量   
    Ns = 50;%用户数量   
...
%     plotCoordinates(vehicles, users, servers);%在二维坐标中画出用户、车辆和BS的初始位置
    ...
    while ~converged
        ...
        for userIndex = random_user_order       
            b_old = b;
            [b, expected_utility] = play_offloading_game_nonProsTheor(b,expected_utility,b_userIndex_locationIndex,userIndex,avg_rate_userIndex_locationIndex,power,numBS,numVehicles,params.kn,params.fj,params.f_j_R, params.T_j_tol, params.E_j_tol,params.F_j_V, params.cj,params.Dj,params.E_i_p,Di_,t_stay,params.an) 
            for locationIndex = 1:(numBS + numVehicles)
                if locationIndex <= numBS
                    O_userIndex_locationIndex(userIndex,locationIndex) = (b(userIndex,locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex) + params.cj(userIndex,1) * b(userIndex,locationIndex) / params.f_j_R) / params.T_j_tol(userIndex) + ...
                   (power * b(userIndex,locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex)) / params.E_j_tol(userIndex);
        ...
        if flag == Ns 
           ...
           save('nonProsTheor_expected_utility_converging.mat', 'expected_utility_converging');
           save('nonProsTheor_Probability_converging.mat', 'Probability_converging');
           disp(['当前迭代次数: ' num2str(iterations)]);  
        end
    end
