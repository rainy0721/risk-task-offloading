function [ u] = utility_function_nonProsTheor(x, userIndex, b, avg_rate_userIndex_locationIndex, power, numBS, numVehicles,k,fj,f_j_R,T_j_tol,E_j_tol,F_j_V, cj,E_i_p,a)
    ...
    for locationIndex = 1:(numBS + numVehicles)
        ...
        if locationIndex > numBS
           ...
           f_j_v(locationIndex - 2) = ( 1 - (sum(b_replaced(setdiff(1 : Ns, userIndex), locationIndex)) +  x(locationIndex) )/ Di_(locationIndex - 2) ) * F_j_V(locationIndex - 2);
           ...
           if sum(b_replaced(:,locationIndex)) / Di_(locationIndex - 2) >=1
               ...
               O_userIndex_locationIndex(userIndex,locationIndex) = ((x(locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex) + cj(userIndex,1) * x(locationIndex) / ...
                   (( 1 - (sum(b_replaced(setdiff(1 : Ns, userIndex), locationIndex)) +  x(locationIndex) )/ Di_(locationIndex - 2) ) * F_j_V(locationIndex - 2)))/ T_j_tol(userIndex) + ...
                   power * x(locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex)/ E_j_tol(userIndex)) *  (1 - (sum(b_replaced(setdiff(1 : Ns, userIndex), locationIndex)) +  x(locationIndex) )/ Di_(locationIndex - 2)) + ...
                   ((sum(b_replaced(setdiff(1 : Ns, userIndex), locationIndex)) +  x(locationIndex) )/ Di_(locationIndex - 2)) * ((x(locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex)/ T_j_tol(userIndex) + ...
                   power * x(locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex)/ E_j_tol(userIndex)) + cj(userIndex,1) * x(locationIndex) / fj(userIndex,1) / T_j_tol(userIndex) + 1e-16 * cj(userIndex,1) * x(locationIndex) * (fj(userIndex,1))^2 / E_j_tol(userIndex));
           end
           if sum(b_replaced(:,locationIndex) )/ Di_(locationIndex - 2) >=1
               ...
               E_PT_userIndex_locationIndex(userIndex,locationIndex - 2) = (cj(userIndex,1) * x(locationIndex)/ T_j_tol(userIndex) / fj(userIndex) + ...
                   ...
                   cj(userIndex,1) * x(locationIndex) / (( 1 - (sum(b_replaced(setdiff(1 : Ns, userIndex), locationIndex)) +  x(locationIndex) )/ Di_(locationIndex - 2) ) * F_j_V(locationIndex - 2))) / T_j_tol(userIndex) + ...
                   x(locationIndex) * power / avg_rate_userIndex_locationIndex(userIndex,locationIndex) / E_j_tol(userIndex))) .^ a(userIndex) * ( 1 - ( (sum ( b_replaced(setdiff(1 : Ns, userIndex), locationIndex)) +  x(locationIndex)) / Di_(locationIndex - 2) )) - ...
                   k(userIndex) * ((sum ( b_replaced(setdiff(1 : Ns, userIndex), locationIndex) ) +  x(locationIndex)) / Di_(locationIndex - 2))  * ( x(locationIndex) / avg_rate_userIndex_locationIndex(userIndex,locationIndex)/ T_j_tol(userIndex) + ...
                   x(locationIndex) * power / avg_rate_userIndex_locationIndex(userIndex,locationIndex) / E_j_tol(userIndex)) .^ a(userIndex); 
           end
        end   
    end
    
    overall_head = sum( O_userIndex_locationIndex(userIndex,1:6) );
